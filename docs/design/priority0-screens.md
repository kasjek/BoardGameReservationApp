...
- Own profile: roll avatar, **customize cosmetics** (XP unlocks from unique games played, layered on DiceBear), and **multi-select up to 3** favorite categories from [BGG board game categories](https://boardgamegeek.com/browse/boardgamecategory).
- **API:** `GET /api/auth/me`, `GET /api/users/{id}`, `POST /me/avatar/roll`, `GET /api/avatar/cosmetics`, `PATCH /me/avatar/cosmetics`, `PATCH /me/favorite-categories`, `GET /api/bgg/categories`.
