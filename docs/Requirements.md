...
- **FR-B5 (Must)** Each table exposes a clear status to all parties: `Requested`, `Available`, `Confirmed & unpaid`, `Confirmed & paid`, `cancelled` (and `completed` after it ends). **The venue must confirm availability first**; other users can only book seats once the status is `Available`. Reaching minimum players moves the table to `Confirmed & unpaid`; it becomes `Confirmed & paid` only after every reserved seat has paid (bring-your-own tables skip the fee and go straight to `Confirmed & paid`). *(33, 24, 30; decisions 2, 7)*
...
- **FR-E5 (Should)** Users can send private 1:1 messages to each other in the app, without exchanging phone numbers or email. *(12)*
